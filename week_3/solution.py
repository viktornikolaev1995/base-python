...
def check_brand(j, type):
    if j and isinstance(j, str) and j[0].isupper() and not (
            j.replace(".", "0").isdigit() or j.isdigit()) \
            and j != type:
        brand = j
        return brand
...
def check_photo_file_name(j):
    if j and isinstance(j, str) and (".jpeg" in j or ".png" in j or ".gif" in j or ".jpg" in j) and len(
            j.split(".")) == 2 \
    ...
    with open(csv_filename) as csv_fd:
        ...
        for row in reader:
            if "car" in row:
                ...
                a_ = {}
                if len(row) == 7:
                    if row[1] and not (row[1].replace(".", "0").isdigit() or row[1].isdigit()) and row[
                        1] != "spec_machine":
